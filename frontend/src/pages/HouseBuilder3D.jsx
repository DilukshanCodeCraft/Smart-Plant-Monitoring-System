...
export default function HouseBuilder3D() {
  const initialObjs = useMemo(() => {
    try {
      const d = localStorage.getItem('hb3d_v3_auto') || localStorage.getItem('hb3d_v3');
      if (d) {
  ...
  useEffect(()=>{
    localStorage.setItem('hb3d_v3_auto', JSON.stringify({ objects:objs, ts:new Date().toISOString() }));
  },[objs]);
  ...
  const save = () => {
    ...
    localStorage.setItem('hb3d_v3', JSON.stringify(d));
